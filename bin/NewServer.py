...
class NewServer(gtk.Window):
	...
	def add_server(self,widget):
		...
		self.liststore.append([False,hostname,'--','--','--','--','--','--','--'])
	...
	def saveTodb(self,data):
		...
		c.execute('''
			...
			(hostname text,port text,username text,password text)
			'''
		)
		c.execute('''
			insert into monit(hostname,port,username,password)
			values("%s","%s","%s","%s")
