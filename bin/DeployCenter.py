...
class DeployCenter(gtk.VBox):
	...
	def create_model(self):
		liststore=Model.CreateServerModel()
		return liststore
	...
	def create_bottom(self):
		...
		fixed=gtk.Fixed()
		install_button=gtk.Button("开始部署")
		install_button.set_size_request(100,25)
		#install_button.set_relief(gtk.RELIEF_NONE)
		install_button.connect('clicked',self.start_install)
		fixed.put(install_button,0,0)
	...
	def add_server(self,widget):
		NewRole(self.model,self.father)
		return
	...
	def start_install(self,widget):
		...
		if paths is not None:
			for path in paths:
				...
				T=util.INSTALL(model,treeiter,self.model)
				T.setDaemon(True)
				T.start()
		return
